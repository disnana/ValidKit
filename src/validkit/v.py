...
class Validator:
    ...
    def when(self, condition: Callable[[Dict[str, Any]], bool]):
        self._when_condition = condition
        return self

    def _validate_base(self, value: Any, data: Dict[str, Any] = None) -> Any:
...
class StringValidator(Validator):
    ...
    def validate(self, value: Any, data: Dict[str, Any] = None) -> str:
...
class NumberValidator(Validator):
    def __init__(self, type_cls: Union[Type[int], Type[float]]):
        super().__init__()
    ...
    def validate(self, value: Any, data: Dict[str, Any] = None) -> Union[int, float]:
...
class ListValidator(Validator):
    def __init__(self, item_validator: Union[Validator, Dict, Type]):
        super().__init__()
        self._item_validator = item_validator

    def validate(self, value: Any, data: Dict[str, Any] = None) -> List:
        ...
        for i, item in enumerate(value):
            try:
                result.append(validate_internal(item, self._item_validator, data, path_prefix=f"[{i}]"))
            except Exception as e:
                # Re-raise or collect will be handled by validate_internal/caller
                raise
        return self._validate_base(result, data)

class DictValidator(Validator):
    def __init__(self, key_type: Type, value_validator: Union[Validator, Dict, Type]):
        super().__init__()
        self._key_type = key_type
        self._value_validator = value_validator

    def validate(self, value: Any, data: Dict[str, Any] = None) -> Dict:
        ...
        for k, v in value.items():
            ...
            try:
                result[k] = validate_internal(v, self._value_validator, data, path_prefix=f"{k}")
            except Exception as e:
                raise
        return self._validate_base(result, data)
...
class OneOfValidator(Validator):
    ...
    def validate(self, value: Any, data: Dict[str, Any] = None) -> Any:
...
class VBuilder:
    ...
    def list(self, item_validator: Union[Validator, Dict, Type]) -> ListValidator:
        return ListValidator(item_validator)

    def dict(self, key_type: Type, value_validator: Union[Validator, Dict, Type]) -> DictValidator:
